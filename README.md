...
- `microblossom-circuit-level-d{3,9}-{standalone-coyote,qshell}-{u280,v80}-{source,synth,implementation}`: matched comparison targets. Every standalone/QShell pair aliases the same generated application source, including decoder RTL, circuit graph identity, current QSH2/MBQ1 record frontend, timing constraints, and divide-by-two application clock. The packed SystemVerilog `GRAPH_ID` is generated per graph with digest byte 0 in AXI-stream bits 7:0;

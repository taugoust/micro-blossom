...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=MICROBLOSSOM_SKIP_CBINDGEN");
    ...
    // Native MicroBlossom tools depend on this crate as a Rust library and do
    // not consume its generated C header. Allow that package to avoid a nested
    // `cargo metadata` invocation, which otherwise selects this crate's
    // independent Cargo.lock rather than the native crate's vendored lock.
    if env::var_os("MICROBLOSSOM_SKIP_CBINDGEN").is_none() {

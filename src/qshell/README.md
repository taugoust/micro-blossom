...
`CoyoteProcessBeatLink` connects the Rust adapter to the separately packaged `microblossom-qshell-coyote-bridge`. The production Coyote and XDB APIs accept a requested receive length and report final operation completion, but expose neither a trustworthy completed record length nor source AXI-stream `tkeep`/`tlast`. Each bridge therefore posts one fixed normal-response descriptor supplied by the graph contract, waits for final completion, validates the staged header as that exact correction shape, and only then exposes bytes. Logical beat sidebands returned to Rust are reconstructed from the fixed descriptor; they are not source-side observations. This path relies on the active resident QShell correction validator and frame adapter as the trusted boundary: QShell stores the complete record, validates framing/identity/ownership, and completes frame commit before host egress. A 72-byte in-band shell error is unsupported and rejected because the resident RTL does not emit it and the transports have no trusted completed-length metadata; supporting variable errors requires a future authenticated out-of-band record-length/error mechanism. The abstract `QshellRecordLink` error codec remains transport-independent and is not evidence that such errors can traverse these production bridges. Its device-free package self-test and packaged Coyote/xdb behavioral workload pass. `microblossom-d3-qshell-coyote-run` composes this real bridge with the canonical native Rust primal workload for physical execution against `/dev/coyote_fpga_0_v0`. The physical U280 path on `rose` passes the canonical correction with 14 completed operations after `reconfigure-app` loads the timing-clean partial. Twenty consecutive fresh host processes pass without reconfiguration; persistent-connection jobs and induced fault recovery remain separate.
...
The CPU-assisted V80 application uses graph-sized fixed QShell records per decode rather than exposing MBQ1 MMIO commands to the host. Circuit d3 requests/results occupy 2/3 beats; circuit d9 requests/results occupy 13/56 beats. Request schema `0x00020003` carries `MBJ1`, protocol version 1, a sorted defect list bounded by the generated graph contract, and the full graph SHA-256. Result schema `0x00020004` carries `MBR1`, status, graph identity, accelerator-operation count, and a graph-bounded correction-edge list. Both records set `END_OF_ROUND`; the response reverses source and destination while retaining context, round, route capability, and record sequence.
...
The packaged co-processor host executable derives the exact fixed request/result sizes from its immutable graph contract, emits one complete end-of-round request through the Coyote process bridge, and validates the returned result. After that request is submitted, any receive timeout, framing, identity, or in-band-error fault poisons the session and permits no retransmission; another decode requires a newly established transport epoch and route capability. Locally rejected graph/defect inputs remain retryable only because no request beat was submitted.

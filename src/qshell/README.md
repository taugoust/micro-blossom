...
The packaged `microblossom_d3_coprocessor` host executable emits the canonical request through the existing Coyote process bridge and verifies correction edge `[2]`. The host-driven U280/V80 packages remain available under their existing names; no fine-grained MMIO operation crosses PCIe in the CPU-assisted package.

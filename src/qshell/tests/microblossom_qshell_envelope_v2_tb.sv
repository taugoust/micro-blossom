...
microblossom_qshell_envelope_v2 dut (.*);
...
function automatic logic [383:0] header(
    ...
    begin
        ...
        value[QSHELL_V2_CONTEXT_ID_LSB +: QSHELL_V2_CONTEXT_ID_W] = CONTEXT_ID;
        value[QSHELL_V2_ROUND_ID_LSB +: QSHELL_V2_ROUND_ID_W] = ROUND_ID;
        value[QSHELL_V2_SCHEMA_ID_LSB +: QSHELL_V2_SCHEMA_ID_W] = schema;
        ...
        value[QSHELL_V2_ROUTE_CAPABILITY_ID_LSB +: QSHELL_V2_ROUTE_CAPABILITY_ID_W] =
            CAPABILITY_ID;
        value[QSHELL_V2_ROUTE_VERSION_LSB +: QSHELL_V2_ROUTE_VERSION_W] = ROUTE_VERSION;
...
task automatic check_response(
    ...
    begin
        ...
        assert(m_axis_tdata[QSHELL_V2_PAYLOAD_BYTES_LSB +: QSHELL_V2_PAYLOAD_BYTES_W] == 64)
...
initial begin : test
    ...
    send_record(header(0, QSHELL_V2_SCHEMA_MICROBLOSSOM_COMMAND_V1, 0), command, 6'h11);
    ...
    send_record(header(0, QSHELL_V2_SCHEMA_MICROBLOSSOM_COMMAND_V1, 1), command, 6'h12);
    ...
    send_record(
        header(QSHELL_V2_FLAG_END_OF_ROUND,
               QSHELL_V2_SCHEMA_MICROBLOSSOM_COMMAND_V1, 2),
        command,
        6'h13
    );
    ...
    begin
        logic [511:0] first;
        first = '0;
        first[383:0] = header(0, QSHELL_V2_SCHEMA_MICROBLOSSOM_COMMAND_V1, 0);
    ...
    send_record(header(0, QSHELL_V2_SCHEMA_MICROBLOSSOM_COMMAND_V1, 0), command, 6'h15);
    ...
    send_record(header(0, QSHELL_V2_SCHEMA_HELIOS_PHENOM_SYNDROME_V1, 1), command, 6'h16);
    repeat (3) @(posedge aclk);
    assert(!mbq_s_axis_tvalid && s_axis_tready)
        else $fatal(1, "schema rejection did not recover at record boundary");

    $display("MICROBLOSSOM_QSHELL_ENVELOPE_V2_PASS");
    $finish;
end
